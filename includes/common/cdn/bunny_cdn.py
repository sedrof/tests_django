...
def add_countries(url: str, a: list, b: list) -> str:
    """
    ...
    if a:
        url += (
            {1: "?", 0: "&"}[urllib.parse.urlparse(url).query == ""]
            + "token_countries="
            + a
        )
    if b:
...
def sign_url(
    ...
    if parameters:
        for value in parameters:
            ...
            parameter_data_url += (
                value + "=" + urllib.parse.quote("".join(parameters[value]), safe="")
            )
    ...
    hashable_base = (
        ...
        + expires
        + parameter_data
        + {1: user_ip, 0: ""}[user_ip is not None]
    )
